...
def stock_data_out(sec_id, mainboard, long_name=''):
    ...
    r = requests.get(f'https://iss.moex.com/iss/engines/stock/markets/shares/securities/'
                     f'{sec_id}.json')
    if len(json.loads(r.text)['marketdata']['data']) == 0:
        return f'🛑 Эмитент "{long_name}" прекратил свою работу на Мосбирже'
    index = -1
    for i, board in enumerate(json.loads(r.text)['marketdata']['data']):
    ...
    current_price = json.loads(r.text)['marketdata']['data'][index][12]  # 5350
    prev_price = json.loads(r.text)['securities']['data'][index][3]  # 5270
    if long_name == '':
        name = json.loads(r.text)['securities']['data'][index][2]
    else:
        name = long_name  # Магнит ао
    sec_id = json.loads(r.text)['securities']['data'][index][0]  # MGNT
    time_date = str(json.loads(r.text)['marketdata']['data'][index][48])  # 2022-08-18 19:05:00 [32]


    output_text = '<b>' + name + ' • #' + sec_id.replace('-', '_') + '</b>\n'
    if json.loads(r.text)['marketdata']['data'][index][49] is None:
        output_text += 'Цена: <b>' + str(current_price) + '₽</b>\n'
    else:
        output_text += 'Цена: <b>' + str(json.loads(r.text)['marketdata']['data'][index][49]) + '₽</b>\n'
...
def currency(message):
    r = requests.get(f'http://iss.moex.com/iss/statistics/engines/currency/markets/selt/rates.json')
...
def stocks(message):
    ...
    if re.match(r'^[a-zA-Z-]*$', input_text) and (len(input_text) <= 5 or input_text.find('-') > -1):
        ...
        bot.send_message(message.chat.id, stock_data_out(nearest[0][2], nearest[0][3], long_name=nearest[0][0]), parse_mode='html')
            ...
            if (difflib.SequenceMatcher(a=line[2].lower(), b=input_text.lower())).ratio() > nearest[0][1]:
                if nearest[0][1] > 0.71:
                    ...
                    nearest[0][0], nearest[0][1], nearest[0][2], nearest[0][3] = line[2], (difflib.SequenceMatcher(
                        a=line[2].lower(), b=input_text.lower())).ratio(), line[1], line[5]
            if len(nearest) > 9:
                output_text += f'❗️Эмитентов со схожим названием слишком много, будут предложены ' \
